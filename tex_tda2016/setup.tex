...
Using our metrics, we consider the relative LOC and Fan-In values between the introduced and removed versions as interest. \revised{We calculate the interest per SATD instance.} For example, if arbitrary metric values in the introduced and removed versions are 10 and 20 \revised{in the method where the SATD exists}, the relative size is 100 (i.e, $100* \frac{(20-10)}{10}$). In cases where the SATD is not yet removed, we use the numbers from the latest version of JMeter. \revised{I changed this since it makes more sense to divide by the initial `borrowed' value. Sorry} \revised{Our assumption here is that if the SATD incurs positive interest, then it will be more difficult to remove in the future, e.g., if the code becomes more complex compared to when the debt was taken, then it will be more difficult to deal with.}
...
\begin{figure*}[!t]
  ...
  \caption{Distribution of SATD Interest in JMeter. Interest is Measured Using LOC and Fan-In. \todo{Is the y-axis correct?}}
  \label{fig:dist}
  \end{center}
\end{figure*}
