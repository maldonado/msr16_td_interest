...
idx <- order(interest, decreasing = T)
write.csv(file="b.csv", data.CountInput.positive[idx,])
data.CountLine.positive$interest[idx]
data.CountLine.positive$Remove_Comment[idx]

# updated part to generate figures
#ant <- interest[fc=="ant"]
#hadoop <- interest[fc=="hadoop"]
#jmeter <- interest[fc=="jmeter"]
#log4j <- interest[fc=="log4j"]
#tomcat <- interest[fc=="tomcat"]
#beanplot(ant,hadoop,jmeter,log4j,tomcat)
#boxplot(ant,hadoop,jmeter,log4j,tomcat)
